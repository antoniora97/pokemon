...
class Batalla(val pokemon1 : Pokemon, val pokemon2 : Pokemon) {
    ...
    fun RecibirObjeto(poke:Pokemon, curacion:Int){ poke.curar(curacion) }
       ...
       return 1.0 }
        ...
        private fun ejecutarAccion (atacante: Pokemon, enemigo: Pokemon) {
            ...
            when (accion.lowercase()) {
                "atacar" -> {
                    val datos = atacante.Atacar(atacante.Movimientos())

                    //todo hacer la leyenda de los indices

                    //datos[4] se refiere a la precisión según la estructura que le hemos dado
                    if (ComprobarAcierto(datos[4].toInt(), enemigo.evasion)){ //Comprueba que el ataque acierta
                        when(datos[2]){ //datos[2] se refiere a la categoría en nuestra estructura
                            "Fisico"-> danio = calcularDanio(atacante.currenAttack, enemigo.currentDefense, datos[1].toInt(), datos[3], enemigo.Type())
                            "Especial" -> danio = calcularDanio(atacante.currentSpAttack,enemigo.currentSpDefense, datos[1].toInt(), datos[3], enemigo.Type())
                        }
                        enemigo.recibirDanyo(danio) //Hace to.do el cálculo del daño
                        ActivarHabilidad(atacante, enemigo, datos[5].toInt()) //Activa el efecto del movimiento
                ...
                "usar objeto" ->  RecibirObjeto(atacante,atacante.UsarObjeto(atacante.Objects()))
